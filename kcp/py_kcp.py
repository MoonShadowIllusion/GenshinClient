...
# 报文段结点
class segment_node:
    def __init__(self, conv1: int, conv2: int):
        self.m_next = None
        self.m_front = None
        self.m_conv1 = conv1
        self.m_conv2 = conv2
    ...
    def log(self):
        ...
        return pattern_str % (
            self.m_index, self.m_wind_size, self.m_time_stamp, self.m_sequence, self.m_receive_next, self.m_resend_tm,
            self.m_rto, self.m_data)
...
# kcp
class py_kcp:
    def __init__(self, conv1: int, conv2: int, udp_socket, out_put_fun, write_log_fun=None):
        self.m_cmd = 0
        self.m_conv1 = conv1
        self.m_conv2 = conv2
        self.m_udp_socket = udp_socket
        ...
        self.m_log_mask = 0
        self.m_cur_seg = segment_node(conv1, conv2)
        self.m_cur_seg.m_conv1 = conv1
        self.m_buff_array: list[bytes] = []
        self.m_array_len = 0
    ...
    def can_log(self, mask):
        return self.m_log_mask & mask and self.m_write_log_fun is not None
    ...
    def output_data(self):
        self.m_out_put_fun(self.m_udp_socket, b''.join(self.m_buff_array))

    # 打包 8 位
    def pack_8bit(self, value, buff_array) -> None:
        # 无符类型
        if value > 127:
    ...
    # 解包 8 位
    def unpack_8bit(self, value, start_index):
        # 暂不考虑负数
        size = start_index + TYPE_8_LEN
    ...
    # 解包 16 位
    def unpack_16bit(self, value, start_index):
        # 暂不考虑负数
        size = start_index + TYPE_16_LEN
    ...
    # 解包 32 位
    def unpack_32bit(self, value, start_index):
        # 暂不考虑负数
        size = start_index + TYPE_32_LEN
        return struct.unpack('I', value[start_index:size])[0], size

    # 打包 seg
    def pack_seg(self, seg: segment_node):
        self.pack_32bit(seg.m_conv1, self.m_buff_array)
        self.pack_32bit(seg.m_conv2, self.m_buff_array)
    ...
    def flush_data(self):
        ...
        # 发送 ack
        for i in range(self.m_ack_count):
            self.deal_send_data()
        ...
        while self.value_diff(self.m_send_next, self.m_ack_knowledge + cwnd) < 0:
            ...
            one_node.m_user_key = self.m_conv1
    ...
    def create_node(self):
        return segment_node(self.m_conv1, self.m_conv2)

    # 用户发送数据
    def send_data(self, data):
        print(f"client_sequence_id = {Global.get_value(Global.client_sequence_id)}")
        Global.set_value(Global.client_sequence_id, Global.get_value(Global.client_sequence_id) + 1)
        # 流数据 index 都为 0
    ...
    # 解析数据
    def parse_data(self, new_node: segment_node):
    ...
    # 向链表指定结点前添加结点
    def add_node(self, link: segment_link, cur_node, new_node):
        # 链表为空、当前结点为尾结点
        if self.link_empty(link) or cur_node == link.m_tail:
            self.add_tail(link, new_node)
        # 当前结点为空，表示向头添加
        elif cur_node is None or cur_node == link.m_head:
    ...
    def input_data(self, data):
        ...
        while True:
            if surplus_length < IKCP_OVERHEAD:
                break
            # 用户 key
            cur_seg.m_conv1, use_length = self.unpack_32bit(data, use_length)
            cur_seg.m_conv2, use_length = self.unpack_32bit(data, use_length)
            if cur_seg.m_conv1 != self.m_conv1 \
                    and cur_seg.m_conv2 != self.m_conv2:
                print("user key error")
                return -1
            cur_seg.m_cmd, use_length = self.unpack_8bit(data, use_length)
            cur_seg.m_index, use_length = self.unpack_8bit(data, use_length)
            cur_seg.m_wind_size, use_length = self.unpack_16bit(data, use_length)
            cur_seg.m_time_stamp, use_length = self.unpack_32bit(data, use_length)
            cur_seg.m_sequence, use_length = self.unpack_32bit(data, use_length)
            cur_seg.m_receive_next, use_length = self.unpack_32bit(data, use_length)
            data_len, use_length = self.unpack_32bit(data, use_length)
            ...
            if cur_seg.m_cmd == IKCP_CMD_ACK:
                ...
                if self.can_log(IKCP_LOG_IN_ACK):
                    self.m_write_log_fun(self.m_udp_socket,
                                         "sn=%d rtt=%d rto=%d" % (cur_seg.m_sequence, rtt, self.m_rto))
                if not flag:
                    flag = True
                ...
                if self.value_diff(cur_seg.m_sequence, self.m_receive_next + self.m_receive_win) < 0:
                    ...
                    if self.value_diff(cur_seg.m_sequence, self.m_receive_next) >= 0:
                        ...
                        new_seg.m_conv1 = cur_seg.m_conv1
                        new_seg.m_conv2 = cur_seg.m_conv2
                        ...
                        if data_len > 0:
                            new_seg.m_data = data[use_length:data_len + use_length]
